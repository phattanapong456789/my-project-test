  ...
  <div class="admin-page">
    <div class="admin-card">
      <div class="admin-header">
        ...
        <div class="header-actions">
          <router-link to="/admin/floor-plan" class="btn-outline">
      ...
      <div class="summary-grid">
        ...
        <div class="summary-card s-pending">
          <div class="s-icon" style="color:var(--orange)">
            <Clock :size="24" />
          </div>
        ...
        <div class="summary-card s-approved">
          <div class="s-icon" style="color:var(--green)">
            <CheckCircle2 :size="24" />
          </div>
          <div class="s-num">{{ shownApproved }}</div>
          <div class="s-label">ยืนยันแล้ว</div>
        </div>
        <div class="summary-card s-tables">
      ...
      <div class="filters">
        ...
        <button @click="clearFilter" class="btn-clear">
          <X :size="13" /> ล้าง
        </button>
      </div>
      ...
      <template v-else>
        <div v-if="selectedIds.length > 0" class="bulk-actions">
          ...
          <button @click="deleteSelected" class="btn-delete-bulk" :disabled="deleting">
            <Trash2 :size="13" /> ลบรายการที่เลือก
          </button>
        </div>
        ...
        <div class="res-list">
          ...
          <div v-for="r in groupedReservations" :key="r.id" class="res-row" :class="`status-${r.status}`">
            ...
            <div class="res-info">
              ...
              <div class="res-meta">
                <span>
                  <User :size="12" /> {{ r.user.name }} ({{ r.user.email }})
                </span>
                <span>
                  <CalendarDays :size="12" /> {{ formatDT(r.reserved_at) }}
                </span>
              </div>
              ...
              <div v-if="r.admin_note" class="res-admin-note">
                <MessageSquare :size="12" /> ทางร้าน: {{ r.admin_note }}
              </div>
            </div>
            <div class="res-actions" v-if="r.status === 'pending'">
              <button @click="openAction(r, 'approved')" class="btn-approve">
                <CheckCircle2 :size="13" /> อนุมัติ
              </button>
              <button @click="openAction(r, 'rejected')" class="btn-reject">
    ...
    <div v-if="deleteConfirm" class="modal-overlay" @click.self="deleteConfirm = false">
      <div class="modal">
        ...
        <div class="modal-actions">
          ...
          <button @click="confirmDelete" class="btn-danger" :disabled="deleting">{{ deleting ? 'กำลังลบ...' : 'ยืนยันลบ'
            }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="actionTarget" class="modal-overlay" @click.self="actionTarget = null">
      <div class="modal">
        ...
        <p><strong>{{ actionTarget.user.name }}</strong> จอง <strong>โต๊ะ {{actionTarget.tables.map(t =>
          t.number).join(',') }}</strong><br>{{ formatDT(actionTarget.reserved_at) }}</p>
        ...
        <div class="modal-actions">
          ...
          <button @click="confirmAction" :class="actionStatus === 'approved' ? 'btn-approve-confirm' : 'btn-danger'"
            :disabled="actioning">{{
