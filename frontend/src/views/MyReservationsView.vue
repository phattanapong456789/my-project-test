  ...
  <div class="page">
    <div class="container">
      <div class="header">
        ...
        <router-link to="/booking" class="btn-new">
          <Plus :size="14" /> จองโต๊ะใหม่
        </router-link>
      </div>
        ...
        <div v-for="r in groupedReservations" :key="r.id" class="res-card" :class="`s-${r.status}`">
            ...
            <div class="res-table">
              ...
              <div>
                ...
                <span class="seats-tag">
                  <Users :size="10" /> {{r.tables.reduce((sum, t) => sum + t.seats, 0)}} ที่นั่ง
                </span>
              </div>
            </div>
          ...
          <div class="res-details">
            <div class="detail-row">
              <CalendarDays :size="13" /><span>{{ formatDate(r.reserved_at) }}</span>
            </div>
            <div v-if="totalPrice(r) > 0" class="detail-row price-row">
              <Banknote :size="13" /><span><strong>ราคา : ฿{{ totalPrice(r).toLocaleString('th-TH', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) }}</strong></span>
            </div>
            ...
            <div v-if="r.note" class="detail-row">
              <FileText :size="13" /><span>{{ r.note }}</span>
            </div>
            <div v-if="r.admin_note" class="admin-note">
              <MessageSquare :size="13" /><span>ทางร้าน: {{ r.admin_note }}</span>
            </div>
          </div>

          <div class="res-footer">
            <span class="created-at">จองเมื่อ {{ formatCreated(r.created_at) }}</span>
            <button v-if="r.status === 'pending'" @click="cancelRes(r)" class="btn-cancel"
...
import { ArrowLeft, Plus, CalendarOff, Armchair, Users, Clock, CheckCircle2, XCircle, Ban, CalendarDays, FileText, MessageSquare, X, Banknote } from 'lucide-vue-next'
